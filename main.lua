...
local hum, _hum = char:FindFirstChildWhichIsA('Humanoid')
local bindable_event --Used for hooking reset button.
...
local function movedir_calculation(move_direction)
    if not ((move_direction * settings['Dv Amplifier']).Magnitude > 30) then
        ...
        while wait() do
            ...
            if (move_direction * settings['Dv Amplifier']).Magnitude > 30 then
                warn('Dv Amplifier wasnt set high enough to maitain parts. Dv Amplifier was set to: ' ..tostring(settings['Dv Amplifier']))
...
local function rotvel_calculation(rot_velocity)
...
local function stabilize(part, part_to, cframe)
    ...
    print(string.format('Stabilizing: ' ..tostring(part_to.Name)))
end
...
do --hat renaming function in lua numbers. <hat, hat1, hat2>
    local hat_names = {}
...
--do scope is used so that tables like hat_names gets garbage collected.
...
for _,accessory in pairs(_hum:GetAccessories()) do
    local real_accessory = char:FindFirstChild(accessory.Name)

    if not real_accessory then accessory:Destroy() end

    local handle = accessory:FindFirstChildWhichIsA('BasePart')
    local real_handle = real_accessory:FindFirstChildWhichIsA('BasePart')

    if not handle or not real_handle then continue end

    handle.Transparency = 1

    local accessory_weld = real_handle:FindFirstChildWhichIsA('Weld')
    ...
    local mesh = handle:FindFirstChildWhichIsA('SpeicalMesh')
